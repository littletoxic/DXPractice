...
internal partial struct Scene {
    ...
    internal readonly unsafe PtrSpan<Animation> Animations => new(mAnimations, mNumAnimations);

    internal readonly unsafe bool HasAnimations() => mAnimations != null && mNumAnimations > 0;
}
...
internal partial struct NodeAnim {
    internal readonly unsafe ReadOnlySpan<VectorKey> PositionKeys => new(mPositionKeys, (int)mNumPositionKeys);

    internal readonly unsafe ReadOnlySpan<VectorKey> ScalingKeys => new(mScalingKeys, (int)mNumScalingKeys);

    internal readonly unsafe ReadOnlySpan<QuatKey> RotationKeys => new(mRotationKeys, (int)mNumRotationKeys);
}

internal partial struct AssimpQuaternion {
    public static implicit operator Quaternion(AssimpQuaternion q) => new(q.x, q.y, q.z, q.w);
}

internal partial struct Animation {
    internal readonly unsafe PtrSpan<NodeAnim> Channels => new(mChannels, mNumChannels);
}