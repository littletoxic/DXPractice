...
internal sealed class Camera {
    ...
    internal Matrix4x4 WorldMatrix => _modelMatrix;
...
internal sealed class Light {
    internal Vector3 LightDirection { get; set; } = new(1, -1, 1);               // 光源发出的光线方向，默认 (1, -1, 1)
    internal Vector3 LightColor { get; set; } = new(1, 1, 1);                    // 光源颜色 (亮度)，默认白色
    internal Vector3 AmbientLightColor { get; set; } = new(0.1f, 0.1f, 0.1f);    // 环境光的颜色 (亮度)，环境光很微弱
}
...
internal sealed class DX12Engine {
    ...
    private const string ModelTextureFilePath = "sakiko/";
    ...
    private struct CBufferLight {
        ...
        internal Vector4 WorldLightDirection;
        internal Vector4 WorldLightColor;
        internal Vector4 AmbientLightColor;
    }
    private ID3D12Resource _cbvLightResource;
    private nint _lightBuffer;

    private ComPtr<ID3D12RootSignature> _rootSignature;

    private static readonly PCSTR Position = AllocatePCSTR("POSITION");
    private static readonly PCSTR Normal = AllocatePCSTR("NORMAL");
        ...
        for (var i = 0; i < modelScene.mNumMeshes; i++) {
            ...
            for (var j = 0; j < mesh.mNumVertices; j++) {
                var newVertex = new Vertex {
                    ...
                    Normal = new(mesh.Normals[j], 0.0f)
                };
    ...
    private unsafe void STEP14_CreateModelTextureResource() {
        ...
        for (var i = 0; i < _materialGroup.Count; i++) {
            ...
            barrier.Transition.pResource = (ID3D12Resource_unmanaged*)_materialGroup[i].DefaultTexture.UnmanagedPointer;
    ...
    private unsafe void STEP18_CreateCBVResource() {
        ...
        var globalLightWidth = CeilToMultiple((uint)Unsafe.SizeOf<CBufferLight>(), D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT);

        cbvResourceDesc.Width = globalLightWidth;
        ...
        _d3d12Device.CreateCommittedResource(
            ...
            D3D12_RESOURCE_STATE_GENERIC_READ,
            null,
            out _cbvLightResource);

        _cbvLightResource.Map(0, null, out var cbvLightPointer);
        _lightBuffer = (nint)cbvLightPointer;
    }
    ...
    private unsafe void STEP19_CreateRootSignature() {
        ...
        // Para 1: (Type = Root Descriptor,  2 DWORD)  (b1, space0) CBV 根描述符，用于全局光源信息
        // Para 2: (Type = Root Constants,   9 DWORD)  (b2, space0) CBV 根常量，用于设置每种材质的光照信息
        // Para 3: (Type = Descriptor Table, 1 DWORD)  (t0, space0) SRV 描述符表，用于纹理
        // 
        // Sampler 0: (Type = Static Sampler) (s0, space0) 静态采样器，用于纹理采样


        var rootParameters = stackalloc D3D12_ROOT_PARAMETER[4];
        ...
        var cbvRootDescriptorDesc2 = new D3D12_ROOT_DESCRIPTOR() {
            ShaderRegister = 1,
            RegisterSpace = 0,
        };
        ...
        rootParameters[1] = new D3D12_ROOT_PARAMETER() {
            ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL,
            ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV,
            Descriptor = cbvRootDescriptorDesc2,
        };


        var cbvRootConstantsDesc = new D3D12_ROOT_CONSTANTS() {
            Num32BitValues = 9,
            ShaderRegister = 2,
            RegisterSpace = 0,
        };
        ...
        rootParameters[2] = new D3D12_ROOT_PARAMETER() {
            ...
            Constants = cbvRootConstantsDesc,
        };
        ...
        rootParameters[3] = new D3D12_ROOT_PARAMETER() {
    ...
    private void UpdateConstantBuffer() {
        ...
        ref var lightBuffer = ref Unsafe.AsRef<CBufferLight>(_lightBuffer);
        lightBuffer.WorldMatrix = _firstCamera.WorldMatrix;
        lightBuffer.CameraPosition = _firstCamera.EyePosition.AsVector4Unsafe();

        _globalLight.LightDirection = Vector3.Transform(_globalLight.LightDirection, Matrix4x4.CreateRotationY(0.01f * MathF.PI / 2f));

        lightBuffer.WorldLightColor = _globalLight.LightColor.AsVector4Unsafe();
        lightBuffer.WorldLightDirection = _globalLight.LightDirection.AsVector4Unsafe();
        lightBuffer.AmbientLightColor = _globalLight.AmbientLightColor.AsVector4Unsafe();
    }
    ...
    private unsafe void Render() {
        ...
        _commandList.ClearRenderTargetView(_rtvHandle, Black);
        ...
        _commandList.SetGraphicsRootConstantBufferView(1, _cbvLightResource.GetGPUVirtualAddress());
