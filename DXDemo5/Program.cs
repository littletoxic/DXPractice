...
internal class DX12Engine {
    ...
    private unsafe void UpdateConstantBuffer() {
        ModelMatrix = Matrix4x4.CreateRotationY(30.0f);
        ViewMatrix = Matrix4x4.CreateLookAtLeftHanded(EyePosition.AsVector3(), FocusPosition.AsVector3(), UpDirection.AsVector3());
        ProjectionMatrix = Matrix4x4.CreatePerspectiveFieldOfViewLeftHanded(XM_PIDIV4, 4.0f / 3, 0.1f, 1000);

        Unsafe.AsRef<CBuffer>((void*)mvpBuffer).MVPMatrix = ModelMatrix * ViewMatrix * ProjectionMatrix;
    }
