...
internal sealed class DX12Engine {
    ...
    private unsafe void CopyDefaultTextureToDefaultResource(int index) {
        ...
        var r = (byte)(Math.Clamp(color.X, 0f, 1f) * 255);
        var g = (byte)(Math.Clamp(color.Y, 0f, 1f) * 255);
        var b = (byte)(Math.Clamp(color.Z, 0f, 1f) * 255);
        var a = (byte)(Math.Clamp(color.W, 0f, 1f) * 255);
    ...
    private unsafe void STEP14_CreateModelTextureResource() {
        ...
        foreach (var material in _materialGroup) {
            ...
            barrier.Transition.pResource = (ID3D12Resource_unmanaged*)material.DefaultTexture.UnmanagedPointer;
